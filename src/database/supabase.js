...
export async function generateTicketId() {
  ...
  try {
    ...
    if (error) {
      console.error("G�� Error generate ticket_id:", error.message);
    ...
    console.log(`G�� Generated Ticket ID: ${newTicketId}`);
    return newTicketId;

  } catch (err) {
    console.error("G�� Unexpected error in generateTicketId:", err);
...
export async function saveEmailLog(email, analysis = {}, telegramSent = false, telegramMessageId = null, telegramChatId = null) {
  try {
    ...
    const payload = {
      ...
      project: analysis.project ||email.project ||null,
    ...
    if (error) {
      console.error("G�� Gagal menyimpan ticket:", error.message);
    ...
    console.log(`G�� Ticket berhasil disimpan: ${ticketId}`);
    return ticketId;

  } catch (err) {
    console.error("G�� Unexpected error in saveEmailLog:", err.message);
    return null;
  }
}
...
export async function saveRawTelegramDM(dmData) {
  try {
    ...
    if (error) {
      console.error('G�� Gagal menyimpan raw Telegram DM:', error.message);
      console.error('   Payload:', JSON.stringify(payload, null, 2));
      return null;
    }

    console.log(`   G�� Raw DM tersimpan ke Supabase: ${ticketId}`);
    return { ticket_id: ticketId, ...payload };

  } catch (err) {
    console.error('G�� Unexpected error in saveRawTelegramDM:', err.message);
    return null;
  }
}
...
export async function updateIncidentStatus(telegramMessageId, newStatus) {
  if (!telegramMessageId) {
    console.warn("G��n+� telegramMessageId kosong, update status dibatalkan");
    return false;
  }
  ...
  if (error) {
    console.error(`G�� Gagal update status menjadi "${newStatus}":`, error.message);
    return false;
  }

  console.log(`G�� Status tiket diubah menjadi "${newStatus}" (telegram_message_id: ${telegramMessageId})`);
  ...
  if (["Done", "Resolved", "Cancelled"].includes(newStatus)) {
    try {
      ...
      console.error("G��n+� Gagal menutup sesi conversation saat update status:", err.message);
    }
  }
...
export async function getTicketById(ticketId) {
  if (!ticketId) {
    console.warn("G��n+� ticketId kosong");
    return null;
  }
  ...
  if (error) {
    if (error.code === 'PGRST116') {
      console.warn(`G��n+� Ticket dengan ID ${ticketId} tidak ditemukan`);
      return null;
    }
    console.error(`G�� Gagal mengambil ticket ${ticketId}:`, error);
...
/**
 * Mencari tiket berdasarkan:
 * 1. ticket_id exact
 * 2. internal id
 * 3. empat digit/angka terakhir ticket_id
 *
 * Contoh:
 * 2389
 * TCK-20260729-2389
...
export async function getTicketByReference(reference) {
  ...
  if (!cleanReference) {
    console.warn("⚠️ Referensi tiket kosong");
    return null;
  }

  if (!/^[A-Z0-9-]+$/.test(cleanReference)) {
    throw new Error(
      `Referensi tiket "${cleanReference}" mengandung karakter yang tidak valid.`
    );
  }

  const candidates = [];

  function addCandidates(rows, matchedBy) {
  ...
  // 1. Cari ticket_id secara exact
  const {
    data: exactTicketIdRows,
    error: exactTicketIdError,
  } = await supabase
    .from("Unified_Ticket_Tracker")
    .select("*")
    .eq("ticket_id", cleanReference)
    .limit(2);

  if (exactTicketIdError) {
    throw new Error(
      `Gagal mencari ticket_id: ${exactTicketIdError.message}`
    );
  }
  ...
  // Jika referensi berupa angka, cari juga internal id
  // dan akhiran ticket_id.
  if (/^\d+$/.test(cleanReference)) {
    const numericId = Number(cleanReference);

    // 2. Cari kolom internal id
    const {
      data: internalIdRows,
    ...
    if (internalIdError) {
      throw new Error(
        `Gagal mencari internal ID: ${internalIdError.message}`
      );
    }
    ...
    // 3. Cari ticket_id yang berakhiran -2389
    const {
      data: suffixRows,
      error: suffixError,
    } = await supabase
    ...
    if (suffixError) {
      throw new Error(
        `Gagal mencari akhiran ticket_id: ${suffixError.message}`
      );
    }
  ...
  /*
   * Jika ID 2389 cocok dengan internal id satu tiket
   * tetapi juga cocok dengan suffix ticket_id tiket lain,
   * jangan memilih secara otomatis.
   */
  if (uniqueResults.length > 1) {
    const matches = uniqueResults
      .map(({ ticket, matchedBy }) => {
        return (
          `${ticket.ticket_id || "-"} ` +
          `(internal id: ${ticket.id}, match: ${matchedBy})`
        );
      })
      .join(", ");

    throw new Error(
      `Referensi "${cleanReference}" cocok dengan lebih dari satu tiket: ${matches}. ` +
      "Gunakan Ticket ID lengkap agar tidak salah update."
    );
  }
...
/**
 * Mengambil tiket berdasarkan kolom internal id.
 *
 * Contoh:
 * [2389] -> Unified_Ticket_Tracker.id = 2389
 */
// export async function getTicketByInternalId(internalId) {
//   const numericId = Number(internalId);

//   if (!Number.isInteger(numericId) || numericId <= 0) {
//     console.warn(
//       `⚠️ Internal ID tiket tidak valid: ${internalId}`
//     );
...
//   if (error) {
//     console.error(
//       `❌ Gagal mengambil tiket berdasarkan internal ID ${numericId}:`,
//       error.message
//     );
...
//   if (!data) {
//     console.warn(
//       `⚠️ Tiket dengan internal ID ${numericId} tidak ditemukan`
//     );
...
export async function updateTicket(ticketId, updateData) {
  if (!ticketId || !updateData || Object.keys(updateData).length === 0) {
    console.warn("G��n+� Data update tidak valid");
    return false;
  }
  ...
  if (error) {
    console.error(`G�� Gagal update ticket ${ticketId}:`, error.message);
    return false;
  }

  console.log(`G�� Ticket ${ticketId} berhasil diupdate`);
  return true;
}
...
export async function deleteTicket(ticketId) {
  ...
  if (error) {
    console.error(`G�� Gagal menghapus ticket ${ticketId}:`, error.message);
    return false;
  }

  console.log(`=���n+� Ticket ${ticketId} berhasil dihapus`);
  return true;
}
...
export async function getTicketsByStatus(status = null) {
  ...
  if (error) {
    console.error("G�� Gagal mengambil tiket berdasarkan status:", error.message);
    return [];
  }
...
export async function getTicketsByDateRange(days = 7) {
  ...
  if (error) {
    console.error(`G�� Gagal mengambil tiket ${days} hari terakhir:`, error.message);
    return [];
  }
...
export async function getTicketsByDate(dateStr, limit = 15) {
  try {
    ...
    if (error) {
      console.error(`G�� Gagal mengambil tiket tanggal ${dateStr}:`, error.message);
      return [];
    }

    return data || [];
  } catch (err) {
    console.error('G�� Unexpected error in getTicketsByDate:', err.message);
    return [];
  }
}
...
export async function searchTickets(keyword) {
  ...
  if (error) {
    console.error("G�� Gagal mencari tiket:", error);
    throw new Error(`Database error: ${error.message || error.code || 'Unknown error'}`);
  }
...
export async function findActiveTicketsForGroup(groupId, source) {
  try {
    ...
    if (error) {
      console.error(`G�� Error mencari tiket aktif grup (${groupId}):`, error.message);
      return [];
    }

    return data || [];
  } catch (err) {
    console.error("G�� Unexpected error in findActiveTicketsForGroup:", err);
    return [];
  }
}
  ...
  try {
    ...
    if (quotedStanzaId) {
      ...
      if (data && data.length > 0) {
        ...
        if (["Done", "Resolved", "Cancelled"].includes(ticket.status)) {
          console.log(`G��n+� Tiket induk ${ticket.ticket_id} ditemukan via quote tetapi statusnya adalah ${ticket.status} (CLOSED). Skip threading.`);
        } else {
    ...
    if (convData && convData.length > 0 && convData[0].ticket_id) {
      ...
      if (ticketData && ticketData.length > 0) {
        ...
        if (["Done", "Resolved", "Cancelled"].includes(ticket.status)) {
          console.log(`G��n+� Sesi OPEN tapi tiket ${ticket.ticket_id} sudah ${ticket.status} (CLOSED). Skip threading.`);
        } else {
          return ticket;
        }
      }
...
export async function saveRawEmail(email) {
  try {
    ...
    if (error) {
      console.error("G�� Gagal simpan email:", error.message);
      return null;
    }

    console.log("G�� Email berhasil masuk intake_message:", data.id);
 ...
 * Dipanggil SEBELUM proses AI apapun G�� setiap pesan masuk selalu disimpan dulu.
 *
...
export async function saveRawIntakeMessage(data) {
  try {
    ...
    if (error) {
      ...
      if (error.code === '23505') {
        console.warn(`G��n+�  Pesan duplikat (idempotency_key: ${payload.idempotency_key}) G�� dilewati`);
        return null;
      }
      console.error('G�� Gagal menyimpan raw intake message ke intake_message:', error.message);
      return null;
    }
    ...
    console.error('G�� Unexpected error in saveRawIntakeMessage:', err.message);
    return null;
  }
}
...
export async function getRawIntakeMessageById(rawId) {
  try {
    ...
    if (error) {
      console.error(
        `G�� Gagal mengambil intake_message dengan ID ${rawId}:`,
        error.message
      );
      return null;
    }
    ...
    console.error(
      `G�� Unexpected error saat mengambil intake_message ID ${rawId}:`,
      err.message
    );
    return null;
  }
...
export async function markRawMessageAs(rawId, status, ticketId = null, ignoreReason = null) {
  try {
    ...
    if (error) {
      console.error(`G�� Gagal markRawMessageAs (id: ${rawId}, status: ${status}):`, error.message);
      return false;
    }

    console.log(`   =��� intake_message[${rawId}] G�� status: ${status}${ticketId ? `, ticket: ${ticketId}` : ''}`);
    return true;
  } catch (err) {
    console.error('G�� Unexpected error in markRawMessageAs:', err.message);
    return false;
  }
}
...
export async function findActiveTicketsByGroupId(groupId, source = null) {
  try {
    ...
    if (error) {
      console.error(`G�� Error findActiveTicketsByGroupId (group: ${groupId}):`, error.message);
      return [];
    }
    ...
    console.error('G�� Unexpected error in findActiveTicketsByGroupId:', err.message);
    return [];
  }
}
...
export async function getPendingRawMessages(limit = 50) {
  try {
    ...
    if (error) {
      console.error('G�� Gagal mengambil pending raw messages:', error.message);
      return [];
    }

    return data || [];
  } catch (err) {
    console.error('G�� Unexpected error in getPendingRawMessages:', err.message);
    return [];
  }
}
...
// G��G�� SLA KONFIRMASI: Ambil tiket Pending Confirmation yang intake_received_at sudah ada
