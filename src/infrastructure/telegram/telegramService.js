...
import { chatWithAI, extractTicketFields } from '../ai/openaiService.js';
import {
  createSession,
  getSession,
  destroySession,
...
async function sendIncidentAlert(email, analysis = {}, customMessage = null) {
  ...
  let messageText = customMessage || email.formalMessage || createFormalTicket(email, analysis);
...
async function handleStatusChange(query, chatId, newStatus) {
  ...
  let messageText = query.message.text || query.message.caption || "";
  ...
  // Coba update teks pesan (Markdown atau HTML)
  try {
    // Ambil teks HTML dari message
    const htmlText = query.message.html_text || query.message.text || "";
    await bot.editMessageReplyMarkup({ inline_keyboard: [] }, {
      chat_id: chatId,
      message_id: messageId
    });
  } catch (err) {
    console.warn("⚠️ Tidak bisa edit pesan status:", err.message);
  }
  ...
  await bot.sendMessage(chatId, `📌 Status tiket diubah menjadi: <b>${escapeHTML(statusDisplay)}</b>`, { parse_mode: 'HTML' });
}
